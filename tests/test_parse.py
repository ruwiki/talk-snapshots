...
CASES = [
    ("ruwiki/2026-07-01.wikitext", "ruwiki", "Википедия:К удалению/1 июля 2026", 21),
    ("ruwiki/2024-03-15.wikitext", "ruwiki", "Википедия:К удалению/15 марта 2024", 22),
    ("enwiki/afd_syria.wikitext", "enwiki", "5 September 2016 Syria bombings", 1),
    ("enwiki/afd_stepper.wikitext", "enwiki", "3rd Ward Stepper", 1),
]
...
def test_every_timestamp_accounted_for(fname, dbname, title, expected_noms):
    ...
    timestamps = len(wiki.locale.timestamp_re().findall(text))
...
def test_ru_thread_chain():
    ...
    noms = parse_page(load(CASES[0][0]), wikis.get("ruwiki"), CASES[0][2])
...
def test_en_votes_parsed():
    noms = parse_page(load("enwiki/afd_syria.wikitext"), wikis.get("enwiki"), "5 September 2016 Syria bombings")
...
def test_bots_flagged_not_dropped():
    noms = parse_page(load("ruwiki/2024-03-15.wikitext"), wikis.get("ruwiki"), CASES[1][2])
